...
// 0) 카테고리 등록. 이 사이트는 글 frontmatter가 아니라 src/categories.ts의 posts 배열로
//    분류하므로, 등록을 빠뜨리면 /category/<slug>/ 목록에 글이 영영 안 나온다.
//    빌드 전에 넣어야 이번 빌드 결과물에 반영된다.
const CATEGORY_RULES = [
  ['vietnam', /베트남|다낭|호치민|하노이|호이안|나트랑|푸꾸옥/],
  ['japan', /일본|오사카|도쿄|교토|후쿠오카|삿포로|오키나와/],
];
const categoriesPath = join(root, 'src', 'categories.ts');
let categoriesSrc = readFileSync(categoriesPath, 'utf8');

if (new RegExp(`'${slug}'`).test(categoriesSrc)) {
  console.log(`▶ 카테고리 등록 생략 — 이미 등록된 글입니다(${slug}).`);
} else {
  const hint = `${assignment.topic} ${slug}`;
  const catSlug = assignment.category
    || (CATEGORY_RULES.find(([, re]) => re.test(hint)) || [])[0]
    || 'tips';

  // 해당 카테고리 블록의 posts 배열 맨 앞(최신순)에 slug를 끼워 넣는다.
  const catStart = categoriesSrc.indexOf(`slug: '${catSlug}'`);
  if (catStart === -1) {
    console.error(`FINALIZE_ERROR: src/categories.ts에 '${catSlug}' 카테고리가 없습니다. assignment.json의 category를 확인하세요.`);
    process.exit(1);
  }
  const postsStart = categoriesSrc.indexOf('posts: [', catStart);
  const insertAt = postsStart + 'posts: ['.length;
  categoriesSrc = categoriesSrc.slice(0, insertAt) + `\n\t\t\t'${slug}',` + categoriesSrc.slice(insertAt);
  writeFileSync(categoriesPath, categoriesSrc);

  if (!assignment.category && catSlug === 'tips' && !/tips/.test(hint)) {
    console.warn(`⚠️ 주제에서 카테고리를 못 찾아 기본값 'tips'로 넣었습니다. 의도와 다르면 src/categories.ts를 고치세요.`);
  }
  console.log(`▶ 카테고리 등록: ${catSlug} ← ${slug}`);
}
